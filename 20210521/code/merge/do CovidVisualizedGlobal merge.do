...
if regexm(c(os),"Mac") == 1 {
	...
	merge 1:1 date using "$pathCovidVisualizedGlobal/LANL/global LANL.dta"
}
...
graph export "graph 13 COVID-19 daily deaths, global, reference scenarios, 2021-01-01 on, CI.pdf", replace
 ...
 daily deaths, median or reference scenario, 2021-01-01 on, scenarios

twoway ///
(line DayDeaMeRaA00S00 date, sort lcolor(cyan) lwidth(vthick) lpattern(tight_dot)) /// 1 JOHN
(line DayDeaMeRaA01S00 date, sort lcolor(red)) /// 2 "DELP" mean
(line DayDeaMeSmA02S02 date, sort lcolor(black)) /// 5 IHME mean, reference scenario
(line DayDeaMeSmA02S02 date, sort lcolor(black) lpattern(dash)) /// 6 IHME mean, better scenario
(line DayDeaMeSmA02S02 date, sort lcolor(black) lpattern(dash)) /// 7 IHME mean, worse scenario
(line DayDeaMeRaA03S02 date, sort lcolor(magenta)) /// 8 IMPE mean, reference scenario
(line DayDeaMeRaA03S02 date, sort lcolor(magenta) lpattern(dash)) /// 9 IMPE mean, better scenario
(line DayDeaMeRaA03S02 date, sort lcolor(magenta) lpattern(dash)) /// 10 IMPE mean, worse scenario
(line DayDeaMeRaA04S00 date, sort lcolor(gold)) /// 11 LANL mean
(line DayDeaMeRaA05S00 date, sort lcolor(green)) /// 14 "SRIV" mean
if date >= td(01jan2021)  ///
, xtitle(Date) xlabel(#9, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
...
graph save "graph 14 COVID-19 daily deaths, global, reference scenarios, 2021-01-01 on, CI.gph", replace
graph export "graph 14 COVID-19 daily deaths, global, reference scenarios, 2021-01-01 on, CI.pdf", replace
