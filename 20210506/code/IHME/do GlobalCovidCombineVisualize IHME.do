...
***************************************************************************
* This is "do GlobalCovidCombineVisualize IHME.do"
...
local up_date "2021-05-07"		// update date 

local up_dir  "2021_05_07"		// update directory 
...
// copy https://ihmecovid19storage.blob.core.windows.net/archive/`up_date'/ihme-covid19.zip ihme-covid19.zip 
// needs update to archive/`up_date'/ when it is moved from "the most recent estimates" URL to /archive/`up_date'/
...
* daily cases, worse scenario = S3, CI
...
, xtitle(Date) xlabel(#19, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%12.0fc) labsize(small))  ylabel(, labsize(small) angle(horizontal)) ///
ytitle(Daily cases) title("COVID-19 daily cases, global, IHME, worse scenario", size(medium)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
legend(order(1 "Mean" 2 "Lower" 3 "Upper") rows(1))

graph save "graph 10 COVID-19 daily cases, global worse scenario CI, IHME.gph", replace
graph export "graph 10 COVID-19 daily cases, global worse scenario CI, IHME.pdf", replace
...
* total cases, worse scenario = S3, CI, not available
