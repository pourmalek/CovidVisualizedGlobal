...
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
legend(order(1 "JOHN" 2 "DELP" 3 "IHME" 4 "IMPE" 5 "LANL" 6 "SRIV") size(small) row(1)) ///
subtitle("after 2021-01-01", size(small) color(white)) 
...
subtitle("after 2020-01-01, without JOHN, LANL, with uncertainty", size(small))

graph save "graph 54 COVID-19 daily CFR, global, reference scenarios, 2020-01-01 on, wo JOHN LANL, with CIs.gph", replace
graph export "graph 54 COVID-19 daily CFR, global, reference scenarios, 2020-01-01 on,wo JOHN LANL, with CIs.pdf", replace
