...
foreach region of local list5 {
	...
	* daily deaths, reference scenarios, all time
	...
	, xtitle(Date) xlabel(#29, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid)  ///
	...
	* daily cases or infections, 3 scenarios, 2021
	...
	(line DayCasMeRaA05S00`region' date, sort lcolor(green)) /// 8 "SRIV" mean
	if date >= td(01jan2021)  ///
	, xtitle(Date) xlabel(#17, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid)  ///
	xlabel(, angle(forty_five)) ylabel(, format(%15.0fc) labsize(small))  ylabel(, labsize(small) angle(horizontal)) ///
	...
	"IMPE infections" 8 "SRIV cases") size(small) row(2)) ///
	subtitle("after 2021-01-01", size(small)) ///
	note("Better and worse scenarios: tight dot (|||||) curves; IHME (Better) and IMPE") 
...
qui graph export "graph GLOBAL 91b COVID-19 daily Infection outcomes ratios, GLOBAL 32 scenarios, IHME.pdf", replace
